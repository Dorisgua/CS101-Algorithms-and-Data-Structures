    ...
    while(times--){
        ...
        if(action==1){
            ...
            if(k_i<0||k_i>index||count[k_i]==-1){
                ...
                if(index_after[index]>=0){//index_after[index]=-1
                    index_before[index_after[index]]=index;//新加入数的下一个的上一个是新加入数的位置//index_before[-1]=1
                }
                int ans_before = index_before[index]>=0 ? val[index_before[index]]:(-1);//防止index_before[0]==-1时超出val范围
                //int ans_before =val[index_before[index]];
