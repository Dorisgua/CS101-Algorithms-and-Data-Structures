    ...
    while(left<=times_const){
        ...
        while(right<=shangyige_index){
            if(kaishi.empty()||right!=kaishi.front()){
                ...
                //max=max{max,nums[jieshu.front()],nums[kaishi.front()]};
                //min=min{min,nums[jieshu.front()],nums[kaishi.front()]};
                right=kaishi.front();
                //cout<<"max:"<<max<<endl; 
                //cout<<"min:"<<min<<endl; 
