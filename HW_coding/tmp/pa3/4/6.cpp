...
int main(){
    ...
    if(A==D){//dijistra
        ...
        heap.emplace(0, 1);
        while (!heap.empty()) {
            if(days_need_of_vertices[n]>=Max_strength){
                break;
            }
            auto [d, x] = heap.top();
            // if(times_of_days>=d){
            ...
            // }
            heap.pop();
            if(times_of_days<days_need_of_vertices[x]){
                dis[x]=maxn;
                continue;
            }
            else{
            ...
            for(int i=0;i<nums_of_adj[x];++i){
                if(visited[vertices[x].neighbors[i]]==0){
                    if(vertices[vertices[x].neighbors[i]].w+times_of_days*D<Max_strength){
                        if(vertices[1].w+added_each_time*times_of_days>vertices[vertices[x].neighbors[i]].w){//2
                            if ((dis[x]+1) < dis[vertices[x].neighbors[i]]){
                                dis[vertices[x].neighbors[i]] = dis[x]+1;
                                heap.emplace(dis[vertices[x].neighbors[i]],vertices[x].neighbors[i]);
                            }
                        }
                        else{// if(vertices[1].w+added_each_time*times_of_days<=vertices[vertices[x].neighbors[i]].w){//3
                            if(days_need_of_vertices[vertices[x].neighbors[i]]<=2+(dis[vertices[x].neighbors[i]]-vertices[1].w)/added_each_time){
